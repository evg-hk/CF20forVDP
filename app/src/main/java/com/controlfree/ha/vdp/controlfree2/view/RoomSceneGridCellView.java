...
public class RoomSceneGridCellView extends LinearLayout {
    ...
    private int bottomH = 0, iconH = 0;

    public RoomSceneGridCellView(Context context, int w, int h) {
        super(context);
        this.c = context;
        this.W = w;
        this.H = h;
        ...
        LinearLayout.LayoutParams p_pb = new LinearLayout.LayoutParams(ResolutionHandler.getPaddingW(), ResolutionHandler.getPaddingW());
        ...
        ((LayoutParams)line.getLayoutParams()).height = 2;
        addView(line);

        LinearLayout.LayoutParams p_v = new LinearLayout.LayoutParams(contentW-ResolutionHandler.getPaddingW()*2, LayoutParams.WRAP_CONTENT);
        ...
        LinearLayout.LayoutParams p_ll_d = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, v.getViewH());
        ...
        iconH = (H-name.getViewH()-v.getViewH()-2-ResolutionHandler.getPaddingW()/2)*2/3;
        ((LinearLayout.LayoutParams)pb.getLayoutParams()).width = iconH;
        ((LinearLayout.LayoutParams)pb.getLayoutParams()).height = iconH;
    ...
    public void setData(JSONObject obj){
        try{
                ...
                for(int i=0;i<mtriggerArr.length();i++){
                    ...
                    if(arr2.length()>0){
                        for(int j=0;j<arr2.length();j++) {
                            if (!tpList.contains(arr2.getJSONObject(j).getString("tp"))) {
                                tpList.add(arr2.getJSONObject(j).getString("tp"));
                            }
                        }
                    }
                }
            ...
            if (dataObj.has("control")) {
                JSONArray idArr = new JSONArray();
                JSONArray arr = new JSONArray(dataObj.getString("control"));
                ...
                for(int i=0;i<dList.length();i++){
                    ...
                    if(rid>0){
                        ExImageView iv = new ExImageView(c, rid, iconH, iconH);
